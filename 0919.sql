...
-- 일반적인 SELECT 문에서는 ORDER BY 절이 맨 나중에 수행되기 때문
SELECT ROWNUM, ENAME, SAL
FROM (SELECT * FROM EMP
        ORDER BY SAL DESC)
WHERE ROWNUM <= 3;

-- SELECT 절에 사용하는 서브쿼리 : 단일행 스칼라 서브쿼리라고도 부름 (반드시 하나의 결과만 나와야 함)
SELECT EMPNO, ENAME, JOB, SAL, (SELECT GRADE
                                FROM SALGRADE 
                                WHERE E.SAL BETWEEN LOSAL AND HISAL) 급여등급,
                                DEPTNO, 
                                (SELECT DNAME 
                                FROM DEPT
...
-- TCL : Transection Control Language 트랜잭션 제어와 세션
